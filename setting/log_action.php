...
if($action == 'show'){
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        while($row = mysqli_fetch_array($result)){
            ...
            $txt="";
            if($row['uname']=='admin'){
                $txt="<span class='badge badge-success'>Admin</span>";
            }else{
                $txt="<span class='badge badge-warning'>User</span>";
            }           
            $out.="<tr>
                <td>{$no}</td>
                <td>{$row["Description"]}</td>
                <td class='text-center'>{$txt}</td>
                <td class='text-center'><span class='badge badge-info'>{$row["Date"]}</span></td>
            </tr>";
        }
        $out.="</tbody>";
        $out.="</table>";
